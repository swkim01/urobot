...
import org.urobot.R;
import org.urobot.media.ContentActivity;
import org.urobot.media.ContentBrowserAdapter;
...
public class BrowseActivity extends ListActivity {
	...
	public void switchToRobotController(DeviceIdentity identity) {
		Intent intent = new Intent(this, RobotActivity.class);
		intent.putExtra (RobotActivity.DEVICE_UUID, identity.getUdn().toString());
				...
				new AdapterView.OnItemClickListener() {
					public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
						...
						if (clickedDisplay != null) {
							...
							//Service service2 = clickedDisplay.getDevice().findService(new UDAServiceId("MotionController"));
							Service service2 = clickedDisplay.getDevice().findService(new URobotServiceId("MotionController"));
							if (service2 != null) {
								DeviceIdentity identity = clickedDisplay.getDevice().getIdentity();
								switchToRobotController(identity);
							}
							else {
								Service service3 = clickedDisplay.getDevice().findService(new UDAServiceId("ContentDirectory"));
								if (service3 != null) {
									DeviceIdentity identity = clickedDisplay.getDevice().getIdentity();
