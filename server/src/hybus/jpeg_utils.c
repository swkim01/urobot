...
/******************************************************************************
Description.: yuv2jpeg function is based on compress_yuv422_to_jpeg written by
              Gabriel A. Devenyi.
...
******************************************************************************/
int compress_yuv422_to_jpeg(struct vdIn *vd, unsigned char *buffer, int size, int quality) {
  struct jpeg_compress_struct cinfo;
  struct jpeg_error_mgr jerr;
  JSAMPROW row_pointer[1];
  unsigned char *line_buffer, *Y, *Cb, *Cr;
  int width, height;
  int z;
  int ystep, ylen, cbstep, cblen, crstep;
  static int written;
  ...
  Cb = Y + ((ylen + PAGE_SIZE-1) & PAGE_MASK);
  cbstep = width/2;
  cblen = (width*height)/2;

  Cr = Cb + ((cblen + PAGE_SIZE-1) & PAGE_MASK);
  crstep = width/2;
...
int reverse_compress_yuv422_to_jpeg(struct vdIn *vd, unsigned char *buffer, int size, int quality) {
  struct jpeg_compress_struct cinfo;
  struct jpeg_error_mgr jerr;
  JSAMPROW row_pointer[1];
  unsigned char *line_buffer, *Y, *Cb, *Cr;
