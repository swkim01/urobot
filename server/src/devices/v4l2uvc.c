...
int init_videoIn(struct vdIn *vd, char *device, int width,
                 ...
                 struct urobot_t *ut, struct device_t *dev)
{
    ...
    v4l2priv *priv = (v4l2priv *) calloc(1, sizeof(v4l2priv));
    dev->priv_data  = (char *)priv;

    priv->in_formats = NULL;
    for(priv->formatCount = 0; 1; priv->formatCount++) {
        struct v4l2_fmtdesc fmtdesc;
        fmtdesc.index = priv->formatCount;
        fmtdesc.type  = V4L2_BUF_TYPE_VIDEO_CAPTURE;
        if(xioctl(vd->fd, VIDIOC_ENUM_FMT, &fmtdesc) < 0) {
            break;
        }

        if (priv->in_formats == NULL) {
            priv->in_formats = (input_format*)calloc(1, sizeof(input_format));
        } else {
            priv->in_formats = (input_format*)realloc(priv->in_formats, (priv->formatCount + 1) * sizeof(input_format));
        }
        ...
        memcpy(&priv->in_formats[priv->formatCount], &fmtdesc, sizeof(input_format));

        if(fmtdesc.pixelformat == format)
            priv->currentFormat = priv->formatCount;

        DBG("Supported format: %s\n", fmtdesc.description);
        struct v4l2_frmsizeenum fsenum;
        fsenum.index = priv->formatCount;
        fsenum.pixel_format = fmtdesc.pixelformat;
        int j = 0;
        priv->in_formats[priv->formatCount].supportedResolutions = NULL;
        priv->in_formats[priv->formatCount].resolutionCount = 0;
        priv->in_formats[priv->formatCount].currentResolution = -1;
        while(1) {
            fsenum.index = j;
            j++;
            if(xioctl(vd->fd, VIDIOC_ENUM_FRAMESIZES, &fsenum) == 0) {
                priv->in_formats[priv->formatCount].resolutionCount++;
                if (priv->in_formats[priv->formatCount].supportedResolutions == NULL) {
                    priv->in_formats[priv->formatCount].supportedResolutions =
                        (input_resolution*)calloc(1, sizeof(input_resolution));
                } else {
                    priv->in_formats[priv->formatCount].supportedResolutions =
                        (input_resolution*)realloc(priv->in_formats[priv->formatCount].supportedResolutions, j * sizeof(input_resolution));
                }
                ...
                priv->in_formats[priv->formatCount].supportedResolutions[j-1].width = fsenum.discrete.width;
                priv->in_formats[priv->formatCount].supportedResolutions[j-1].height = fsenum.discrete.height;
                if(format == fmtdesc.pixelformat) {
                    priv->in_formats[priv->formatCount].currentResolution = (j - 1);
...
int v4l2SetControl(struct vdIn *vd, int control_id, int value, struct urobot_t *ut, struct device_t *dev)
{
    struct v4l2_control control_s;
    int min, max;
    int ret = 0;
    ...
    for (i = 0; i<dev->parametercount; i++) {
        if (dev->parameters[i].v4l2ctrl.ctrl.id == control_id) {
            got = 0;
            break;
        }
    }
    ...
    if (got == 0) { // we have found the control with the specified id
        ...
        if (dev->parameters[i].v4l2ctrl.class_id == V4L2_CTRL_CLASS_USER) {
            DBG("Control type: USER\n");
            min = dev->parameters[i].v4l2ctrl.ctrl.minimum;
            max = dev->parameters[i].v4l2ctrl.ctrl.maximum;
            ...
            ext_ctrl.id = dev->parameters[i].v4l2ctrl.ctrl.id;

            switch(dev->parameters[i].v4l2ctrl.ctrl.type) {
...
void control_readed(struct vdIn *vd, struct v4l2_queryctrl *ctrl, struct urobot_t  *ut, struct device_t *dev)
{
    ...
    if (dev->parameters == NULL) {
        dev->parameters = (control*)calloc(1, sizeof(control));
    } else {
        dev->parameters =
        (control*)realloc(dev->parameters,(dev->parametercount + 1) * sizeof(control));
    }
    ...
    memcpy(&dev->parameters[dev->parametercount].v4l2ctrl.ctrl, ctrl, sizeof(struct v4l2_queryctrl));
    dev->parameters[dev->parametercount].v4l2ctrl.group = IN_CMD_V4L2;
    dev->parameters[dev->parametercount].v4l2ctrl.value = c.value;
    if(ctrl->type == V4L2_CTRL_TYPE_MENU) {
        dev->parameters[dev->parametercount].v4l2ctrl.menuitems =
        ...
        for(i = ctrl->minimum; i <= ctrl->maximum; i++) {
            ...
            if(xioctl(vd->fd, VIDIOC_QUERYMENU, &qm) == 0) {
                memcpy(&dev->parameters[dev->parametercount].v4l2ctrl.menuitems[i], &qm, sizeof(struct v4l2_querymenu));
                DBG("Menu item %d: %s\n", qm.index, qm.name);
            } else {
        ...
        dev->parameters[dev->parametercount].v4l2ctrl.menuitems = NULL;
    }

    dev->parameters[dev->parametercount].v4l2ctrl.value = 0;
#ifdef V4L2_CTRL_FLAG_NEXT_CTRL
    dev->parameters[dev->parametercount].v4l2ctrl.class_id = (ctrl->id & 0xFFFF0000);
#else
    dev->parameters[dev->parametercount].v4l2ctrl.class_id = V4L2_CTRL_CLASS_USER;
#endif

    int ret = -1;
    if (dev->parameters[dev->parametercount].v4l2ctrl.class_id == V4L2_CTRL_CLASS_USER) {
        ...
        if(ret) {
            ...
            switch(ctrl->type)
            {
                ...
                case V4L2_CTRL_TYPE_STRING:
                    //string gets set on VIDIOC_G_EXT_CTRLS
                    //add the maximum size to value
                    dev->parameters[dev->parametercount].v4l2ctrl.value = ext_ctrl.size;
                    break;
#endif
                case V4L2_CTRL_TYPE_INTEGER64:
                    dev->parameters[dev->parametercount].v4l2ctrl.value = ext_ctrl.value64;
                    break;
                default:
                    dev->parameters[dev->parametercount].v4l2ctrl.value = ext_ctrl.value;
                    break;
            }
        }
    }
...
void enumerateControls(struct vdIn *vd, struct urobot_t *ut, struct device_t *dev)
{
    // enumerating v4l2 controls
    struct v4l2_queryctrl ctrl;
    v4l2priv *priv = (v4l2priv *) dev->priv_data;
    dev->parametercount = 0;
    dev->parameters = NULL;
    ...
    memset(&priv->jpegcomp, 0, sizeof(struct v4l2_jpegcompression));
    if(xioctl(vd->fd, VIDIOC_G_JPEGCOMP, &priv->jpegcomp) != EINVAL) {
        DBG("JPEG compression details:\n");
        DBG("Quality: %d\n", priv->jpegcomp.quality);
        DBG("APPn: %d\n", priv->jpegcomp.APPn);
        DBG("APP length: %d\n", priv->jpegcomp.APP_len);
        DBG("APP data: %s\n", priv->jpegcomp.APP_data);
        DBG("COM length: %d\n", priv->jpegcomp.COM_len);
        DBG("COM data: %s\n", priv->jpegcomp.COM_data);
        ...
        if (dev->parameters == NULL) {
            ...
            dev->parameters = (control*)realloc(dev->parameters,(dev->parametercount + 1) * sizeof(control));
        }
        ...
        memcpy(&dev->parameters[dev->parametercount].v4l2ctrl.ctrl, &ctrl_jpeg, sizeof(struct v4l2_queryctrl));
        dev->parameters[dev->parametercount].v4l2ctrl.group = IN_CMD_JPEG_QUALITY;
        dev->parameters[dev->parametercount].v4l2ctrl.value = priv->jpegcomp.quality;
        dev->parametercount++;
    } else {
        DBG("Modifying the setting of the JPEG compression is not supported\n");
        priv->jpegcomp.quality = -1;
    }
