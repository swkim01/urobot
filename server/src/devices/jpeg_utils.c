...
int compress_yuyv_to_jpeg(cameraBuffer *cambuf, unsigned char *buffer, int size, int quality)
{
    ...
    line_buffer = (unsigned char *)calloc(cambuf->width * 3, 1);
    yuyv = cambuf->framebuffer;
    ...
    cinfo.image_width = cambuf->width;
    cinfo.image_height = cambuf->height;
    ...
    z = 0;
    while(cinfo.next_scanline < cambuf->height) {
        int x;
        unsigned char *ptr = line_buffer;

        for(x = 0; x < cambuf->width; x++) {
            int r, g, b;
            int y, u, v;
...
int compress_rgb_to_jpeg(cameraBuffer *cambuf, unsigned char *buffer, int size, int quality)
{
    ...
    line_buffer = (unsigned char *)calloc(cambuf->width * 3, 1);
    rgb = (unsigned char *)cambuf->framebuffer;
